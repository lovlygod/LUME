...
const ChatEmojiPicker = ({ onEmojiClick, className }: ChatEmojiPickerProps) => {
  ...
  if (!emojiData) {
    return (
      <div
        className={`w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-2xl flex items-center justify-center h-48 ${className ?? ''}`}
      >
  ...
  return (
    <div
      className={`w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-2xl shadow-[0_12px_40px_rgba(0,0,0,0.35)] overflow-hidden flex flex-col ${className ?? ''}`}
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
          {CATEGORIES.map((cat, i) => (
            <button
              ...
              className={`w-8 h-8 rounded-lg flex items-center justify-center text-base transition-smooth ${
              ...
              <div className="grid gap-1" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(36px, 1fr))' }}>
                {filteredEmojis.map((emoji) => (
                  <button
                    ...
                    className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-white/10 transition-smooth"
                    title={emoji.name}
                  >
                    <img
          ...
          : CATEGORIES.map((cat, i) => {
              ...
              return (
                <div
                  ...
                  className="mb-2"
                >
                  <div className="text-[10px] font-medium text-white/30 px-1 py-1">
                    {cat.label}
                  </div>
                  <div
